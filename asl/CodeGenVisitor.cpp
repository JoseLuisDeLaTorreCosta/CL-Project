...
antlrcpp::Any CodeGenVisitor::visitAssignStmt(AslParser::AssignStmtContext *ctx) {
  ...
  if (Types.isArrayTy(tid1) and Types.isArrayTy(tid2)) {
    ...
    int sizea = (int)Types.getArraySize(tid1);


    code = code || instruction::ILOAD(temp, "0") || instruction::LABEL(labelWhile) 
                || instruction::ILOAD(temp1, std::to_string(sizea))
                || instruction::LT(temp1, temp, temp1) || instruction::FJUMP(temp1, labelEndWhile) 
                || instruction::LOADX(temp1, addr2, temp)
                || instruction::XLOAD(addr1, temp, temp1)
                || instruction::ILOAD(temp1, "1") || instruction::ADD(temp, temp, temp1) 
...
antlrcpp::Any CodeGenVisitor::visitWhileStmt(AslParser::WhileStmtContext *ctx) {
  ...
  std::string labelWhile    = "while" + label;
  std::string labelEndWhile = "endwhile"+label;
  instructionList code = instruction::LABEL(labelWhile)
                      || codeE || instruction::FJUMP(addrE, labelEndWhile)
...
antlrcpp::Any CodeGenVisitor::visitArrLeftExpr(AslParser::ArrLeftExprContext *ctx) {
  ...
  TypesMgr::TypeId    typeEx = getTypeDecor(ctx->expr());

  std::size_t typeSize = Types.getSizeOfType(typeEx);
  std::string temp1 = "%"+codeCounters.newTEMP();
  std::string temp2 = "%"+codeCounters.newTEMP();
  instructionList code = codeId || codeEx
                      || instruction::ILOAD(temp1, std::to_string(typeSize))
